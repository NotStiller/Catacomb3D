...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_ftimebonus,TILEGLOBAL/2);
//	new->tileobject = true;
...
void ExplodeWall (int tilex, int tiley)
{
	...
	objtype *new;
	new = DSpawnNewObj (tilex,tiley,&s_walldie1,0);
	if (new == NULL)
		return;
	...
	if (gnd_colors[gamestate->mapon] == 0x0101)
		...
		tilenum = WALLEXP;
	SetActorAtInt(new->tilex,new->tiley,tilenum);
	SetTileMap(new->tilex,new->tiley,tilenum);
...
void SpawnWarp (int tilex, int tiley, int type)
{
	...
	objtype *new;
	if (type)
		new = SpawnNewObj (tilex,tiley,&s_obj_gate1,TILEGLOBAL/3);
	else
		new = SpawnNewObj (tilex,tiley,&s_pit,TILEGLOBAL/3);
	new->obclass = gateobj;
	new->temp1 = type;
}
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_troll1,35*PIXRADIUS);
	new->speed = 2500;
...
void SpawnWetMan(int tilex, int tiley)
{
	objtype *ob;
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_wet_bubbles1,PIXRADIUS*35);
	ob=new;
...
void SpawnZombie (int tilex, int tiley)
{
	...
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_zombie_inground,35*PIXRADIUS);
	ob = new;
...
foundtile:;
	...
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_wallskel,PIXRADIUS*35);
	ob = new;
...
void T_WallSkeleton(objtype *ob)
{
	...
	switch (wskel_mode)
	{
		...
		case ws_wall1:
		case ws_wall3:
			tile = wskel_base+(wskel_mode-ws_wall1);
			SetTileMap(x,y,tile);
			SetActorAtInt(x,y,tile);
			SetMapSegs(0,x,y,tile);
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_skel_1,PIXRADIUS*35);
...
void SpawnEye(int tilex, int tiley)
{
	...
	new = SpawnNewObj(tilex,tiley,&s_eye_1,PIXRADIUS*35);
	ob = new;
	new->obclass = eyeobj;
	new->speed = 1200;
	new->flags |= of_shootable;
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_orc1,PIXRADIUS*35);
	new->obclass = orcobj;
	new->speed = 1536;
	new->flags |= of_shootable;
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_demon1,PIXRADIUS*35);
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_mage1,PIXRADIUS*35);
	new->obclass = mageobj;
	new->speed = 3072;
	new->flags |= of_shootable;
	new->hitpoints = EasyHitPoints(5);
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_red_demon1,PIXRADIUS*35);
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_grel1,PIXRADIUS*35);
...
{
	objtype *new;
	new = SpawnNewObj(tilex,tiley,&s_bat1,PIXRADIUS*35);
	new->obclass =batobj;
	new->flags |= of_shootable;
...
#warning in the following line, the last ob had been obj before. On purpose ?
		SetActorAt(ob->tilex,ob->tiley,ob);	// set down a new goal marker
	} while (0);	//(move)
	CalcBounds (ob);
}
...
boolean ShootPlayer(objtype *ob, short obclass, short speed, statetype *state)
{
	...
	objtype *new;
	new = DSpawnNewObjFrac (ob->x,ob->y,state,PIXRADIUS*35);
	if (new == NULL) {
		return true;
	}
	new->speed = speed;
